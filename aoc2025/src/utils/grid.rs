    ...
    U: Clone,
{
        ...
        B: Borrow<U>,
    {
        let elements: Vec<((i32, i32), U)> = iter
            ...
            .collect();
        let h = 1 + elements.iter().map(|el| el.0.0).max().unwrap() as usize;
        let w = 1 + elements.iter().map(|el| el.0.1).max().unwrap() as usize;
    ...
    T: Clone,
{
    ...
    pub fn map_with_eight_neighbors<U>(&self, mut fun: impl FnMut(&T, Vec<T>) -> U) -> Grid<U>
    where
        U: Clone,
    {
        let grid = self
...
impl Grid<bool> {
    pub fn print_braille(&self) {
        ...
        for y0 in (0..(self.height() + 3)).step_by(4) {
            for x0 in (0..(self.width() + 1)).step_by(2) {
                // Determine character code
                let mut c: u32 = 0x2800;
                let mut pow = 1;
                for (dy, dx) in [
                    (0, 0),
                    (1, 0),
                    ...
                    if let Some(true) = self.get((y0 + dy, x0 + dx)) {
                        c += pow;
                    }
                    pow *= 2
                }
                chars.push(char::from_u32(c).expect("Broken implementation for braille"));
            }
            chars.push('\n');
        }
        println!("{chars}");
